...
const (
	Prompt = `To help improve the quality of this product, we collect anonymized usage data
for details on what is tracked and how we use this data visit <https://skaffold.dev/docs/telemetry/>. This data is handled in accordance with our privacy policy <https://policies.google.com/privacy>/
...
func DisplayMetricsPrompt(configFile string, out io.Writer) error {
	if isStdOut(out) {
		...
		return updateConfig(configFile)
	}
	return nil
}
